...
Fixtures are entirely synthetic and use a fixed capcode/function. Supported
variants exercise either a 20-character alpha page or the legacy tone-only
path. No received/off-air traffic is used.
"""
...
EXPECTED_SHA256 = {
    (512, "alpha"): "b2609f893c1b73155678a551f83a431be931a1c4fc747eefcb78afd863471584",
    (1200, "alpha"): "9800babd1d2f789a95d1622be3ad47aa9e9fc8597c5a254b9a29bb54514ada70",
    (2400, "alpha"): "a46529d11fd769b78f8710d2bfbfe315089e5f812aeb1ce34171e4562c163db3",
    (1200, "tone"): "7e1fafa0091caabde87a2aa0b18bc0184c29ca32c9b47a555f8bd6a0b49a07b0",
}
...
def build_transmission_bits(kind: str) -> list[int]:
    ...
    if kind == "alpha":
        ...
        # Tone-only is represented by an address followed immediately by IDLE.
        batch = [address_word, *([IDLE] * 15)]
    else:
...
def main() -> int:
    if len(sys.argv) not in (2, 3, 4):
        print(
            f"usage: {Path(sys.argv[0]).name} OUTPUT.wav [512|1200|2400] [alpha|tone]",
            file=sys.stderr,
        )
        return 2
    ...
    kind = sys.argv[3] if len(sys.argv) == 4 else "alpha"
    key = (baud_rate, kind)

    if key not in EXPECTED_SHA256:
        print(f"unsupported POCSAG fixture: baud={baud_rate}, kind={kind}", file=sys.stderr)
        return 2

    output.parent.mkdir(parents=True, exist_ok=True)

    wav = make_wav(build_pcm(build_transmission_bits(kind), baud_rate))
    ...
    print(
        f"generated {output} ({len(wav)} bytes, baud={baud_rate}, kind={kind}, sha256={digest})"
    )
    return 0
