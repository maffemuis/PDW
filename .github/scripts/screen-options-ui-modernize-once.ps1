...
Replace-Exact $anchor $insert 'screen options modern surface insertion'
...
git rm .github/workflows/screen-options-ui-modernize-once.yml .github/scripts/screen-options-ui-modernize-once.ps1
git add $path
git diff --cached --check
if ($LASTEXITCODE -ne 0) { throw 'cached diff check failed' }
git commit -m 'Modernize Screen Options dialog presentation'
