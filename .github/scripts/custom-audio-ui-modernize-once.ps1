...
Replace-Exact $anchor $insert 'custom audio modern surface insertion'
...
git rm .github/workflows/custom-audio-ui-modernize-once.yml .github/scripts/custom-audio-ui-modernize-once.ps1
git add $path
git diff --cached --check
if ($LASTEXITCODE -ne 0) { throw 'cached diff check failed' }
git commit -m 'Modernize Custom Audio dialog presentation'
