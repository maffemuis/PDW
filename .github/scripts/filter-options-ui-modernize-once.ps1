...
Replace-Exact $anchor $insert 'Filter Options modern surface insertion'
...
git rm .github/workflows/filter-options-ui-modernize-once.yml .github/scripts/filter-options-ui-modernize-once.ps1
git add $path
git diff --cached --check
if ($LASTEXITCODE -ne 0) { throw 'cached diff check failed' }
git commit -m 'Modernize Filter Options dialog presentation'
