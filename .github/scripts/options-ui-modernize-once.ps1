...
Replace-Exact $anchor $insert 'options modern surface insertion'
...
git rm .github/workflows/options-ui-modernize-once.yml .github/scripts/options-ui-modernize-once.ps1
git add $path
git diff --cached --check
if ($LASTEXITCODE -ne 0) { throw 'cached diff check failed' }
git commit -m 'Modernize decoder Options dialog presentation'
